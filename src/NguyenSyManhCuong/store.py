        ...
        if self._use_chroma and self._collection is not None:
            try:
                ...
                if existing and existing.get("ids") and len(existing["ids"]) > 0:
                    ...
                    return True
                # Also try checking metadata if doc_id was stored in metadata
                meta_existing = self._collection.get(where={"doc_id": doc_id})
                if meta_existing and meta_existing.get("ids") and len(meta_existing["ids"]) > 0:
                    self._collection.delete(ids=meta_existing["ids"])
                    return True
                return False
            except Exception:
                return False
