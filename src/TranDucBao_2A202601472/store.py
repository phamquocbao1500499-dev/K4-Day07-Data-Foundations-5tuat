...
class EmbeddingStore:
    ...
    def _from_chroma_query(self, raw: dict[str, Any]) -> list[dict[str, Any]]:
        ids = raw.get("ids", [[]])[0]
        documents = raw.get("documents", [[]])[0]
        metadatas = raw.get("metadatas", [[]])[0]
        distances = raw.get("distances", [[]])[0]
        return [
            {"id": doc_id, "content": content, "metadata": metadata or {}, "score": 1 - distance}
    ...
    def search(self, query: str, top_k: int = 5) -> list[dict[str, Any]]:
        ...
        """
        if self._use_chroma and self._collection is not None:
            raw = self._collection.query(query_embeddings=[self._embedding_fn(query)], n_results=top_k)
            return self._from_chroma_query(raw)
